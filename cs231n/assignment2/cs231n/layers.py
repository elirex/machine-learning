...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    # Nor the two transformation
    gammax = gamma * x_hat # (N, D)
...
def batchnorm_backward(dout, cache):
  ...
  dgamma = np.sum(dgammax * x_hat, axis=0) # (D,)
  dx_hat = dgammax * gamma # (N, D)

  divar = np.sum(dx_hat * xmu, axis=0) # (D,)
  dxmu_1 = dx_hat * ivar # (N, D)
  ...
  dxmu_2 = 2 * xmu * dsq # (N, D)

  dx1 = (dxmu_1 + dxmu_2) # (N, D)
          ...
          for hp in xrange(H_prime):
              for wp in xrange(W_prime):
                  out[n, f, hp, wp] = np.sum(
                      x_pad[n, :, hp * stride:hp * stride + HH, wp * stride:wp * stride + WW] * w[f, :]) + b[f]
  cache = (x, w, b, conv_param)
  return out, cache
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def conv_backward_naive(dout, cache):
  ...
  for f in xrange(F):
      for c in xrange(C):
          for hh in xrange(HH):
              for ww in xrange(WW):
                  sub_xpad = x_pad[:, c, hh:hh+H_prime*S:S, ww:ww+W_prime*S:S]
                  dw[f, c, hh, ww] = np.sum(dout[:, f, :, :] * sub_xpad)
              ...
              for f in xrange(F):
                  for hp in xrange(H_prime):
                      for wp in xrange(W_prime):
                          ...
                          if (hx + P - hp * S) < HH and (hx + P - hp * S) >= 0:
                              mask1[:, hx + P - hp * S, :] = 1.0
                          if (wx + P - wp * S) < WW and (wx + P - wp * S) >= 0:
                              mask2[:, :, wx + P - wp * S] = 1.0
                          w_masked = np.sum(
                                  w[f, :, :, :] * mask1 * mask2, axis=(1, 2))
                          dx[n, :, hx, wx] += dout[n, f, hp, wp] * w_masked
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx, dw, db
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in xrange(N):
      for c in xrange(C):
          for h1 in xrange(H1):
              for w1 in xrange(W1):
                  out[n, c, h1, w1] = np.max(
                          x[n, c, h1*S:h1*S+Hp, w1*S:w1*S+Wp])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
      for c in range(C):
          for h1 in range(H1):
              for w1 in range(W1):
                  x_pooling = x[n, c, h1*S:h1*S+Hp, w1*S:w1*S+Wp]
                  maxi = np.max(x_pooling)
                  x_mask = x_pooling == maxi
                  dx[n, c, h1*S:h1*S+Hp, w1*S:w1*S+Wp] += dout[n, c, h1, w1] * x_mask
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
      ...
      # Store the updated ruuning means back into bn_param
      bn_param['running_maen'] = running_mean
      bn_param['running_var'] = running_var
  elif mode == 'test':
